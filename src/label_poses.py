...
ASSET_DIR = 'models/'
OPTIMIZED_MODEL = ASSET_DIR + 'resnet18_baseline_att_368x368_A_epoch_249_trt.pth'
...
def preprocessvideo(activity, path):
    ...
    trackers = []
    while True:
        cv2.namedWindow("Action", cv2.WINDOW_AUTOSIZE)
        ...
        if ret:
            ...
            for tracker in trackers:
                ...
                if len(tracker.q) >= window:
                    contents = list(np.hstack(list(tracker.q)[-window:]))

                    # Preventing all 0 entries
                    all_zeros = True
                    ...
                    if all_zeros == False:
                        newFileWriter.writerow(contents+[activity]) # prevent same lines with the same front

#            # Display while preprocessing
...
if __name__ == "__main__":
    directory = "/data/ntu" # folder containing .mp4 files
    window = 9 # LSTM time window 
    activitylist = ['drink_eat','sit_squat','phone','walk','selfie']
    ...
    dataFile = open(os.path.join(ASSET_DIR, 'preprocessoutput_ntu_r18_368.csv'),'w') # Where output csv will be stored
    newFileWriter = csv.writer(dataFile)

    # Adding Header Row
    numbers = list(range(0, window*36)) # 36 body points from OPTIMIZED_MODEL
    newFileWriter.writerow(numbers + ['y'])

    
    for act in tqdm(activitylist, position=1):
        # go into their file and add in the data
        videofolder = os.path.join(directory, act)

        if not os.path.exists(videofolder):
            print('Error: Video Folder provided does not exist, Activity Names do not match')
        
        # processing every folder
        for file in tqdm(os.listdir(videofolder), position=2):
            path=os.path.join(videofolder, file)
            # print(path)
            preprocessvideo(act, path)
