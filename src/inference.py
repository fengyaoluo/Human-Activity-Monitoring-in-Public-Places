...
name = 'out-labeled.mp4'
fourcc = cv2.VideoWriter_fourcc(*'mp4v')
out = cv2.VideoWriter(name, fourcc, 30.0, (w, h))

import tensorflow as tf
secondary_model = tf.keras.models.load_model('models/lstm_ntu_r18_368.h5', compile = False)
window = window_size
pose_vec_dim = 36
motion_dict = {0: 'drink_eat', 1:'sit_squat', 2:'phone', 3:'walk', 4:'selfie'}
...
while True:
    ...
    if ret:
        ...
        for tracker in trackers:
            tracker.draw_box(image)

        for tracker in trackers:
            if len(tracker.q) >= window_size:
                sample = np.array(list(tracker.q)[:window_size])
